...
for subj = 1:length(cfg.subjects)
    ...
    fMRI_img = {[cfg.outputDirBIDSf 'sub-' cfg.subjects{subj} filesep 'ses-' cfg.sessions{subj} filesep 'func' filesep 'wsub-' cfg.subjects{subj} '_ses-' cfg.sessions{subj} '_task-' cfg.fMRITask '_bold_fALFF.nii,1']}; 
...
for subj = 1:length(cfg.subjects)
    ...
    for atlas = 1:length(cfg.atlas)
        ...
        temp_mv_file = [cfg.outputDirBIDSf 'sub-' cfg.subjects{subj} filesep 'ses-' cfg.sessions{subj} filesep 'func' filesep cfg.atlas{atlas} '_wsub-' cfg.subjects{subj} '_ses-' cfg.sessions{subj} '_task-' cfg.fMRITask '_bold_fALFF.csv'];
...
for atlas = 1:length(cfg.atlas)
    ...
    for subj = 1 %get header info from first subject
        ...
        temp_csv = readcell([cfg.outputDirBIDSf 'sub-' cfg.subjects{subj} filesep 'ses-' cfg.sessions{subj} filesep 'func' filesep cfg.atlas{atlas} '_wsub-' cfg.subjects{subj} '_ses-' cfg.sessions{subj} '_task-' cfg.fMRITask '_bold_fALFF.csv']);
    ...
    writecell(temp_combined,[cfg.spm12output filesep 'ROI_' cfg.atlas{atlas} '_task-' cfg.fMRITask '_bold_fALFF.csv']);
    bnum = bnum+1;
