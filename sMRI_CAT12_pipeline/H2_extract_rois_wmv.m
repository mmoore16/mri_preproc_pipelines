...
for subj = 1:length(cfg.subjects)
    ...
    %copy wm file with different name to avoid overwriting
    temp_in_wMRIfile = [cfg.outputDirBIDSs 'sub-' cfg.subjects{subj} filesep 'ses-' cfg.sessions{subj} filesep 'anat' filesep 'p2sub-' cfg.subjects{subj} '_ses-' cfg.sessions{subj} '_T1w.nii'];
    temp_cp_wMRIfile = [cfg.outputDirBIDSs 'sub-' cfg.subjects{subj} filesep 'ses-' cfg.sessions{subj} filesep 'anat' filesep 'p2sub-' cfg.subjects{subj} '_ses-' cfg.sessions{subj} '_T1w_redo.nii'];
    if ~exist(temp_cp_wMRIfile, 'file') %check if renamed file already exists
        cmd_cpwMRIfile = strcat("cp "+temp_in_wMRIfile+" "+temp_cp_wMRIfile); %move roi file
        system(cmd_cpwMRIfile);
    end
    ...
    wMRI_img = {[cfg.outputDirBIDSs 'sub-' cfg.subjects{subj} filesep 'ses-' cfg.sessions{subj} filesep 'anat' filesep 'p2sub-' cfg.subjects{subj} '_ses-' cfg.sessions{subj} '_T1w_redo.nii']};
    
    sMRI_imgs = cat(1,sMRI_imgs,sMRI_img);
    wMRI_imgs = cat(1,wMRI_imgs,wMRI_img);
    
end

wMRI_imgs = {cat(1,wMRI_imgs)};

matlabbatch{bnum}.spm.tools.cat.tools.ROIsum.Method.ManySubj.field = sMRI_imgs;
matlabbatch{bnum}.spm.tools.cat.tools.ROIsum.Method.ManySubj.images = wMRI_imgs;
...
for subj = 1:length(cfg.subjects)
    ...
    for atlas = 1:length(cfg.atlas)
        ...
        temp_mv_file = [cfg.outputDirBIDSs 'sub-' cfg.subjects{subj} filesep 'ses-' cfg.sessions{subj} filesep 'anat' filesep cfg.atlas{atlas} '_sub-' cfg.subjects{subj} '_ses-' cfg.sessions{subj} '_T1w_redo_wmv.csv'];
...
for atlas = 1:length(cfg.atlas)
    ...
    for subj = 1 
        ...
        temp_csv = readcell([cfg.outputDirBIDSs 'sub-' cfg.subjects{subj} filesep 'ses-' cfg.sessions{subj} filesep 'anat' filesep cfg.atlas{atlas} '_sub-' cfg.subjects{subj} '_ses-' cfg.sessions{subj} '_T1w_redo_wmv.csv']);
    ...
    writecell(temp_combined,[cfg.CAT12output filesep 'ROI_' cfg.atlas{atlas} '_T1w_redo_wmv.csv']);
    bnum = bnum+1;
